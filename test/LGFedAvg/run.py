...
from src.flbase.strategies.LGFedAvg import LGFedAvgClient, LGFedAvgServer
...
def run(partition, beta, num_classes_per_client, num_shards_per_client, yamlfile, use_wandb, keep_clients_model, device):
    ...
    server_config['strategy'] = 'LGFedAvg'
    ...
    clients_dict = setup_clients(LGFedAvgClient, trainset, None, criterion,
    ...
    server = LGFedAvgServer(server_config, clients_dict, exclude=server_config['exclude'],
                          server_side_criterion=criterion, global_testset=testset, global_trainset=trainset,
                          client_cstr=LGFedAvgClient, server_side_client_config=client_config, server_side_client_device=device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Test LGFedAvg.')
    ...
    parser.add_argument('--keep_clients_model', default=False, type=lambda x: (str(x).lower() in ['true', '1', 'yes']), help='Keep LGFedAvg local model')
    parser.add_argument('--device', default='cuda:1', type=str, help='cuda device')
    args = parser.parse_args()
    run(args.partition, args.beta, args.num_classes_per_client, args.num_shards_per_client, args.yamlfile, args.use_wandb, args.keep_clients_model, args.device)
