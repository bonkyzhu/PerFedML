...
def run(partition, beta, num_classes_per_client, num_shards_per_client, yamlfile, use_wandb, keep_clients_model, device):
    ...
    clients_dict = setup_clients(FedAvgClient, trainset, None, criterion,
    ...
    server = FedAvgServer(server_config, clients_dict, exclude=server_config['exclude'],
                          server_side_criterion=criterion, global_testset=testset, global_trainset=trainset,
                          client_cstr=FedAvgClient, server_side_client_config=client_config, server_side_client_device=device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Test FedAvg.')
