...
def run(partition, beta, num_classes_per_client, num_shards_per_client, yamlfile, use_wandb, keep_clients_model, device, debug):
    if debug:
        from src.flbase.strategies.LocalTrain_debug import LocalTrainClient, LocalTrainServer
        print('Debug Mode')
    else:
        from src.flbase.strategies.LocalTrain import LocalTrainClient, LocalTrainServer
    ...
    server_config['strategy'] = 'LocalTrain'
    ...
    clients_dict = setup_clients(LocalTrainClient, trainset, None, criterion,
    ...
    server = LocalTrainServer(server_config, clients_dict, exclude=server_config['exclude'],
                          server_side_criterion=criterion, global_testset=testset, global_trainset=trainset,
                          client_cstr=LocalTrainClient, server_side_client_config=client_config, server_side_client_device=device)
    if debug:
        directory = f"./debug/"
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Test LocalTrain.')
    ...
    parser.add_argument('--keep_clients_model', default=False, type=lambda x: (str(x).lower() in ['true', '1', 'yes']), help='Keep LocalTrain local model')
    parser.add_argument('--device', default='cuda:1', type=str, help='cuda device')
    parser.add_argument('--debug', default=False, type=lambda x: (str(x).lower() in ['true', '1', 'yes']), help='Use wandb pkg')
    args = parser.parse_args()
    run(args.partition, args.beta, args.num_classes_per_client,
        args.num_shards_per_client, args.yamlfile, args.use_wandb,
        args.keep_clients_model, args.device, args.debug)
