...
def run(partition, beta, num_classes_per_client, num_shards_per_client, yamlfile, use_wandb, keep_clients_model, device):
    ...
    clients_dict = setup_clients(FedPerClient, trainset, None, criterion,
    ...
    server = FedPerServer(server_config, clients_dict, exclude=server_config['exclude'],
                          server_side_criterion=criterion, global_testset=testset, global_trainset=trainset,
                          client_cstr=FedPerClient, server_side_client_config=client_config, server_side_client_device=device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Test FedPer.')
    ...
    parser.add_argument('--keep_clients_model', default=False, type=lambda x: (str(x).lower() in ['true', '1', 'yes']), help='Keep FedPer local model')
