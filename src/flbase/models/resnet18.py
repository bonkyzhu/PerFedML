...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, config, num_classes=1000, zero_init_residual=False,
                 groups=1, group_norm_nums=0, width_per_group=64, replace_stride_with_dilation=None):
        super(ResNet, self).__init__()

        self.config = config

        self.group_norm_nums = group_norm_nums
        ...
        temp = nn.Linear(512 * block.expansion, num_classes, bias=False).state_dict()['weight']
    ...
    def _norm_layer(self, num_channels):
        if self.group_norm_nums > 0:
            return nn.GroupNorm(self.group_norm_nums, num_channels)
        else:
    ...
    def _forward_impl(self, x):
        ...
        # self.device = self.cal_gpu()
        # x = deepcopy(x).to(self.device)
        ...
        if self.prototype is None:
            ...
            x = torch.matmul(x, self.prototype.T)
...
def resnet_18_cifar(config):
    ...
    """
    num_classes = _decide_num_classes(config['dataset'])
    
    model = ResNet(block=BasicBlock, layers=[2, 2, 2, 2], config=config,  num_classes=num_classes, 
                   group_norm_nums=config['group_norm_num_groups'])
