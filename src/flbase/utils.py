...
# File: utils.py
# Project: flbase
# Created Date: 2021-12-19 12:47
# Author: Yutong Dai yutongdai95@gmail.com
# -----
# Last Modified: 2022-06-06 7:13
...
def setup_optimizer(model, config, round):
    weight_decay = config["weight_decay"] if "weight_decay" in config.keys() else 1e-5
    ...
    elif config['lr_scheduler'] == 'diminishing':
        lr = config['learning_rate'] * (config['lr_decay_per_round'] ** (round - 1))
    else:
    ...
    if config['optimizer'] == 'SGD':
        ...
        # print(f'weight_decay={weight_decay}, momentum={momentum}')
...
def setup_clients(Client, trainset, testset, criterion, client_config_lst, device, **kwargs):
    ...
    # devices = [cid % gpu_num for cid in range(num_clients)]
    all_clients_dict = {}
