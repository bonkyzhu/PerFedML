...
class FedRepClient(FedAvgClient):
    ...
    def _initialize_model(self):
        ...
        # Prototype is parameter
        self.head_prototype = deepcopy(self.model.prototype)
    ...
    def to_freeze_layers(self, layers_to_freeze):
        ...
        for param in self.model.named_parameters():
            layer_name = param[0]
            model_state_dict[layer_name] = param[1]
            if layer_name in layers_to_freeze:
                model_state_dict[layer_name].requires_grad = False
            else:
                model_state_dict[layer_name].requires_grad = True
...
class FedRepServer(FedAvgServer):
    def __init__(self, server_config, clients_dict, exclude, **kwargs):
        ...
        # FedRep do not aggregate head as implemented in the FedBaBu's code
        ...
        if len(self.exclude_layer_keys) > 0:
            print(f"FedRepServer: the following keys will not be aggregate:\n ", self.exclude_layer_keys)
        freeze_layers = []
        ...
        if len(freeze_layers) > 0:
            print("FedRepServer: the following layers will not be updated:", freeze_layers)
    ...
    def run(self, **kwargs):
        ...
        for r in round_iterator:
            ...
            for cid in self.active_clients_indicies:
                client = self.clients_dict[cid]
                ### initialize head
                client.model.prototype = deepcopy(client.head_prototype)
                ### fix body
                client.to_freeze_layers(self.include_layer_keys)
                ### train head
                client.training(r, client.client_config['head_finetune_round'])
                ### fix head
